  ...
  return (
    <div className="relative w-full max-w-xl mx-auto mb-3 z-10">
      <div className={`relative bg-white border-4 border-black rounded-3xl p-4 shadow-pop transition-all transform duration-300 ${
        ...
        status === 'incorrect' ? 'border-blue-600 bg-rose-50 animate-shake' : ''
      }`}>
        ...
        <div className="flex items-start gap-3">
          ...
          <div className={`relative shrink-0 rounded-2xl p-2.5 border-3 border-black shadow-pop-sm flex items-center justify-center ${
            ...
            status === 'incorrect' ? 'bg-blue-400' :
            'bg-tycoon-green text-white'
          }`}>
          ...
          <div className="flex-1">
            ...
            {/* Hint Box if toggled */}
            {showHint && hintText && (
              <div className="mt-2 bg-blue-100 border-2 border-dashed border-green-600 rounded-xl p-2 flex items-center justify-between text-xs font-bold text-green-900">
                <span className="flex items-center gap-1.5">
                  <Sparkles className="w-4 h-4 text-amber-600" /> HINT GURU: {hintText}
                </span>
                <button 
                  onClick={onToggleInspector}
                  className="bg-green-400 hover:bg-amber-500 text-black border-2 border-black rounded-lg px-2 py-1 text-[11px] font-extrabold"
                >
                  🔍 Kaca Pembesar
                </button>
              </div>
